...
class pdf_propale_huygens extends ModelePDFPropales
{
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "huygens"; // Creator of the wave theory for light
		$this->description = $langs->trans('PDFHuygensDescription');
	...
	* 		@param 	Propal		$object		common object
		...
		if ($conf->propal->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					// Stock
					if ($this->getColumnStatus('qty_comm')) {
						$this->printStdColumnContent($pdf, $curY, 'qty_comm', "");
						$nexY = max($pdf->GetY(), $nexY);
					}
	...
	public function defineColumnField($object, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$this->cols['position'] = array(
			'rank' => $rank,
			'width' => 10,
			'status' => getDolGlobalInt('PDF_HUYGENS_ADD_POSITION') ? true : (getDolGlobalInt('PDF_ADD_POSITION') ? true : false),
			'title' => array(
		...
		if ($this->atleastonediscount) {
			$this->cols['discount']['status'] = false;
		}
		...
		$this->cols['totalexcltax'] = array(
			'rank' => $rank,
			'width' => 26, // in mm
			'status' => !getDolGlobalString('PDF_PROPAL_HIDE_PRICE_EXCL_TAX') ? false : false,
			'title' => array(
		...
		$this->cols['totalincltax'] = array(
			'rank' => $rank,
			'width' => 26, // in mm
			'status' => !getDolGlobalString('PDF_PROPAL_SHOW_PRICE_INCL_TAX') ? false : false,
			'title' => array(
